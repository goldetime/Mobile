...
public class MainActivity extends AppCompatActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    login.setOnClickListener((View v) -> {
			...
			Cursor result = getContentResolver().query(NameProvider.CONTENT_URI, projection, "f_name = ?", new String[] {id}, null);
    ...
    if (requestCode == REQUEST_CODE_1 && resultCode == RESULT_OK) {
      try {
        User t = (User) data.getSerializableExtra("serial");
          ...
          newUser.put("month", mh);
					...
					Uri recordUri = getContentResolver().insert(NameProvider.CONTENT_URI, newUser);
      ...
      try {
        ...
        updateUser.put("day", dy);
				...
				int r = getContentResolver().update(NameProvider.CONTENT_URI, updateUser, "f_name = ?", new String[] {id});
